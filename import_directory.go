...
func (pe *PEFile) parseImports64(importDesc *ImportDescriptor) (err error) {
	...
	for idx, thunk := range table {
		...
		if thunk.Data.AddressOfData > 0 {
			...
			if table[idx].Data.AddressOfData&ordinalFlag > 0 {
				imp.ImportByOrdinal = true
				imp.Ordinal = table[idx].Data.AddressOfData & uint64(0xffff)
			} else {
				imp.ImportByOrdinal = false
				imp.HintNameTableRva = table[idx].Data.AddressOfData & uint64(addressMask)
				...
				tmp, err := pe.getOffsetFromRva(uint32(table[idx].Data.AddressOfData + 2))
				if err != nil {
					return err
				}
				imp.NameOffset = uint64(tmp)
			}
...
// Per https://docs.microsoft.com/en-us/windows/win32/debug/pe-format#import-directory-table the
// rva here is 4 bytes not 8, even for PE32+.
	...
	for rva := startRVA; rva < lastAddr; {
		...
		if thunk.Data.AddressOfData > 0 {
			...
			if thunk.Data.AddressOfData&ordinalFlag > 0 {
				// but its value is beyond 2^16, we will assume it's a
				// corrupted and ignore it altogether
				if thunk.Data.AddressOfData&uint64(0x7fffffff) > uint64(0xffff) {
					msg := fmt.Sprintf("Corruption detected in thunk data at 0x%x", rva)
					log.Printf(msg)
					return nil, errors.New(msg)
				}
				...
				if thunk.Data.AddressOfData > uint64(maxAddressOfData) {
					maxAddressOfData = thunk.Data.AddressOfData
				}
				if thunk.Data.AddressOfData < uint64(minAddressOfData) {
					minAddressOfData = thunk.Data.AddressOfData
				}
			}
		}
